---
name: c07a-s28a-car
description: Grounded workflow for the C07A + S28A H-problem vehicle-mounted ball-balancing car using TI MSPM0G3507, CCS, SysConfig, MSPM0 SDK, DriverLib, K230 HiWonder CanMV/MicroPython, bidirectional UART, and the D36A-driven 42-stepper ball-rod linkage. Use when Codex inspects, creates, modifies, builds, flashes, debugs, or diagnoses this repository's MSPM0 firmware or .syscfg files; writes or diagnoses K230 camera, ball-position vision, display, touch, UART, or HMI code; configures GPIO, UART, I2C, SPI, timers, STEP/DIR/EN, PWM, ADC, or interrupts; diagnoses D36A power, wake, pulse, direction, current, heating, or lost-step faults; integrates the chassis motors, encoders, MPU6050, eight-channel grayscale sensor, OLED, K230, D36A, 42-stepper linkage, or power wiring; or checks C07A/S28A pinmux and board conflicts. Do not apply its board-specific pin assumptions to other MSPM0 boards.
---

# C07A + S28A H-Problem Ball-Balancing Car
...
Always read [references/source-priority.md](references/source-priority.md). For pin or connector work, also read [references/pinmux-summary.md](references/pinmux-summary.md) and [references/s28a-board-map.md](references/s28a-board-map.md). For UART behavior, control conventions, HMI, ISR structure, or bring-up work, read [references/project-architecture.md](references/project-architecture.md). For K230 CanMV/MicroPython, camera, display, vision, touch, UART, or HMI work, read [references/module-docs/k230/index.md](references/module-docs/k230/index.md). For D36A power, STEP/DIR/EN, connectors, DIP settings, motor wiring, or ball-rod mechanics, read [references/module-docs/d36a-ballrod/index.md](references/module-docs/d36a-ballrod/index.md). For one-way K230/MSPM0 UART, `MSP RX:0`, or UART faults introduced while integrating vision and control code, read [references/k230-uart-debugging.md](references/k230-uart-debugging.md) before changing CRC, framing, or pin assignments. For eight-channel grayscale wiring, timing, GPIO, or control work, read [references/module-docs/grayscale/index.md](references/module-docs/grayscale/index.md). For any `.syscfg`, generated-code, DriverLib, build, or debug work, read [references/sysconfig-workflow.md](references/sysconfig-workflow.md). Use [references/official/index.md](references/official/index.md) and [references/board-docs/index.md](references/board-docs/index.md) to locate the bundled TI and board documents.
...
For D36A bring-up, keep EN1 low and STEP stopped while checking power and wiring. Set DIR, drive EN1 high, wait at least 1 ms, then verify PB16 STEP at the source pin and at D36A P1 pin 8 before attaching the mechanical load. A generated timer auto-start setting or successful build is not proof that the physical pulse reaches the driver.
...
- Keep the user-confirmed D36A supply route at raw 7.4 V direct battery feed unless the user explicitly changes the hardware decision. Do not describe chip-level 5.5 V limits as proof of full-module performance at low battery.
- Keep the D36A initial current at approximately 0.55 A until the installed 42-stepper's rated phase current and thermal behavior are established.
