...
Project hardware facts assign PB16/TIMG7_C1 to PAN and PB17/TIMA1_C0 to TILT. A previously verified 48-pin SysConfig solution and generated `ti_msp_dl_config.h` confirmed both routes for its historical build; the currently checked-in active project does not contain that servo configuration. Regenerate and confirm the routes and macro names before implementing them in the active project. If the same servo behaves differently after swapping only PWM signals, first distinguish signal-only swapping from moving the entire three-wire connector. Then check connector continuity, common GND at each servo, rail voltage during startup, and actual PB16/PB17 waveforms before changing direction signs or PID values.

If both outputs are constant low, the servos do not recenter after merging UART/vision code, or MSPM0 status is one-way, follow the explicit counter-start, IRQ-order, oscilloscope, and K230 RX workflow in [K230 UART and gimbal PWM bring-up](../gimbal-k230-uart-debugging.md).
