...
from libs.Utils import * # 包含ALIGN_UP, ScopedTiming
...
class FaceRecognition:
    def __init__(self,face_det_kmodel,face_reg_kmodel,det_input_size,reg_input_size,database_dir,anchors,confidence_threshold=0.25,nms_threshold=0.3,face_recognition_threshold=0.75,rgb888p_size=[1280,720],display_size=[1920,1080],debug_mode=0):
        ...
        self.db_name= []
        self.db_data= []
        # 将 display_size 传递给 FaceDetApp 和 FaceRegistrationApp
...
if __name__=="__main__":
    ...
    # 此处的rgb888p_size表示传感器捕获并传递给AI模型的原始图像分辨率。
    # k230保持不变，k230d可调整为[640,360]
    rgb888p_size = [1280, 720]

    # 根据选择的显示模式初始化显示屏并获取实际显示分辨率
    display_output_width, display_output_height = init_display(select_display)
    # 将实际显示分辨率的宽度进行16字节对齐，以符合硬件要求
    actual_display_size = [ALIGN_UP(display_output_width, 16), display_output_height]
    ...
    # 初始化PipeLine，现在将实际的传感器输入分辨率和最终的显示分辨率传递给它。
    # 假设 PipeLine 能够接受 'display_size' 参数来配置其内部显示输出，
    # 并且如果 Display.init 已经外部调用，PipeLine会适配。
    pl = PipeLine(rgb888p_size=rgb888p_size, display_size=actual_display_size)
    pl.create()

    # 初始化 FaceRecognition 类，传入传感器输入分辨率和实际显示分辨率
    fr = FaceRecognition(
        ...
        display_size=actual_display_size # 使用由 init_display 确定的实际显示分辨率
    )
