...
| Remaining ultrasonic reservation | PA24 | S28A's original ultrasonic header also uses PA9 | Do not enable the original two-pin ultrasonic interface while PA9 drives the buzzer |
...
- PA24 is shared by S28A radar/ultrasonic concepts and remains reserved; PA9 now drives the buzzer, so the original ultrasonic/radar assignment must not be enabled.
...
- PB16/PB17 are fixed to the PAN/TILT servos, PA9 is fixed to the buzzer, and PA22/PA8/PA12 are grayscale address pins. Do not restore the S28A original ultrasonic/CCD/handle defaults over these assignments.
...
- Board-document legacy labels do not override the project hardware assignments: PB16/TIMG7_C1 and PB17/TIMA1_C0 were confirmed in a previously verified generated configuration, but the active project must regenerate and confirm both routes and macro names.
