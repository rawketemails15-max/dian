...
本版本只做静止底盘上的红线定点。默认目标为 `x=160 px`
（`xQ4=2560`），底盘电机始终刹车。程序没有杆角传感器、步进电机
...
4. 恢复 `APP_BALL_CALIBRATION_MODE=0`，确认摄像头红线中心标定值。
   默认是 `APP_BALL_DEFAULT_TARGET_X_Q4=2560`。
5. 放入钢球并短按启动。若 `x<160` 时钢球反而继续向左运动，翻转
...
OLED 在 ACTIVE/HOLD 状态显示当前钢球 X 坐标，故障显示 `Err`。UART
