...
static Q3Telemetry gTelemetry;
static Q3State gState;
...
static void set_state(Q3State state, uint32_t nowMs)
{
    gState = state;
    gStateStartMs = nowMs;
...
static void set_motion_target(
    Q3State state, int32_t targetSteps, uint16_t frequencyHz,
    uint32_t nowMs)
{
    /*
...
static bool state_accepts_gestures(Q3State state)
{
    return (state == Q3_IDLE) ||
        (state == Q3_TRIM) ||
        (state == Q3_COMPLETE);
}

static bool state_is_sequence_running(Q3State state)
{
    switch (state) {
        case Q3_WAKE:
        case Q3_LAUNCH:
...
static void start_timeout_level(uint32_t nowMs)
{
    ...
    set_motion_target(
        Q3_TIMEOUT_LEVEL, gRunZeroSteps, APP_OL_LEVEL_HZ, nowMs);
}
...
static void start_trim_move(uint32_t nowMs)
{
    ...
    if (((gTrimDirection > 0) &&
        ...
        if (gState == Q3_COMPLETE) {
            set_state(Q3_IDLE, nowMs);
        }
        return;
    }
...
void Q3_init(uint32_t nowMs)
{
    ...
    gTelemetry = (Q3Telemetry) {0};
    gTelemetry.state = Q3_IDLE;
...
void Q3_tick_5ms(uint32_t nowMs, bool buttonPressed)
{
    ...
    switch (gState) {
        ...
        case Q3_LAUNCH:
            ...
            if (target_reached()) {
                if (APP_OL_LAUNCH_HOLD_MS == 0U) {
                    set_motion_target(Q3_REVERSE,
                        gRunZeroSteps + APP_OL_REVERSE_STEPS,
                        APP_OL_REVERSE_HZ, nowMs);
                } else {
                    set_state(Q3_LAUNCH_HOLD, nowMs);
                }
            }
            break;
        ...
        case Q3_REVERSE:
            move_toward_target();
            if (target_reached()) {
                set_state(Q3_RETURN_HOLD, nowMs);
            }
            break;
        ...
        case Q3_SETTLE:
            ...
            if (elapsed_ms(nowMs, gStateStartMs,
                ...
                set_state(Q3_COMPLETE, nowMs);
            }
            break;

        case Q3_TIMEOUT_LEVEL:
            move_toward_target();
            if (target_reached()) {
                gFinalRunMs = (uint32_t) (nowMs - gRunStartMs);
                set_state(Q3_FAULT, nowMs);
            }
            break;
