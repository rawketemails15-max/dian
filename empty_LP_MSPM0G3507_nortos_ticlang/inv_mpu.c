...
#define i2c_write   mpu6050_dmp_i2c_write
#define i2c_read    mpu6050_dmp_i2c_read
#define Delay_ms    mpu6050_dmp_delay_ms
#define delay_ms    mpu6050_dmp_delay_ms
#define get_ms      mpu6050_dmp_get_ms
...
uint8_t mpu_dmp_init(void)
{
	...
	{	 
		...
		if(res)return 7;   
		/* Apply the measured bias when factory self-test passes. DMP gyro
		 * calibration remains available for compatible modules that fail it. */
		(void) run_self_test();
...
uint8_t mpu_dmp_get_data(float *pitch, float *roll, float *yaw)
{
	...
	unsigned char more;
	long quat[4];
	uint8_t haveQuaternion = 0U;
	do {
		if(dmp_read_fifo(gyro, accel, quat, &sensor_timestamp, &sensors, &more))return 1;
		if(sensors & INV_WXYZ_QUAT)haveQuaternion = 1U;
	} while(more != 0U);
