    ...
    uint32_t nowMs, const BallVisionSample *vision)
{
    ...
    /*
     * Tiered PD by ball error E (pixels).  Each band uses a distinct KP so
     * the response speed changes noticeably.  Inside the ±5 px band friction
     * compensation is disabled so the breakaway minimum cannot push the ball
     * past the hold-entry line.
     *
     *   |E| ≤  5      KP 0.08   (gentle, settle into hold)
     *   |E| 5 … 25    KP 0.15
     *   |E| 25 … 50   KP 0.20
     *   |E| > 50      KP 0.70   (aggressive catch-up)
     */
    {
        int16_t absErrQ4 = abs_i32(errorQ4);
        float kp, kd;

        if (absErrQ4 <= 80) {
            kp = 0.08f;  kd = 0.010f;
        } else if (absErrQ4 <= 400) {
            kp = 0.15f;  kd = 0.015f;
        } else if (absErrQ4 <= 800) {
            kp = 0.20f;  kd = 0.020f;
        } else {
            kp = 0.70f;  kd = 0.040f;
        }
        ...
        if (absErrQ4 <= 80) {
            command_continuous_tilt(desiredTilt, true);
        } else {
